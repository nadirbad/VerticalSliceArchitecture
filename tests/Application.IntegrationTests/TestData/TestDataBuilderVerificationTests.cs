...
public class TestDataBuilderVerificationTests : IntegrationTestBase
{
    ...
    [Fact]
    public async Task IssuePrescriptionBuilder_DefaultValues_ShouldProduceValidRequest()
    {
        // Arrange
        var builder = new IssuePrescriptionTestDataBuilder();
        var command = builder.Build();
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.Created, "default builder values should create a valid prescription");
    }

    [Fact]
    public async Task IssuePrescriptionBuilder_AsLongTermMedication_ShouldProduceValidRequest()
    {
        // Arrange
    ...
    [Fact]
    public async Task IssuePrescriptionBuilder_AsControlledSubstance_ShouldProduceValidRequest()
    {
        // Arrange
        var command = new IssuePrescriptionTestDataBuilder()
    ...
    [Fact]
    public async Task IssuePrescriptionBuilder_WithInvalidQuantityTooLow_ShouldReturn400()
    {
        // Arrange
        var command = new IssuePrescriptionTestDataBuilder()
    ...
    [Fact]
    public async Task IssuePrescriptionBuilder_WithInvalidRefillsTooHigh_ShouldReturn400()
    {
        // Arrange
        var command = new IssuePrescriptionTestDataBuilder()
    ...
    [Fact]
    public async Task IssuePrescriptionBuilder_WithNonExistentDoctor_ShouldReturn404()
    {
        // Arrange
        var command = new IssuePrescriptionTestDataBuilder()
    ...
    [Fact]
    public void RescheduleAppointmentBuilder_DefaultValues_ShouldProduceValidStructure()
    {
        // Arrange & Act
        var builder = new RescheduleAppointmentTestDataBuilder();
        var (appointmentId, newStart, newEnd, reason) = builder.BuildValues();

        // Assert
        appointmentId.Should().NotBeEmpty("appointment ID should be generated");
        newStart.Should().BeAfter(DateTimeOffset.UtcNow.AddHours(2), "new start should be at least 2 hours in the future");
        newEnd.Should().BeAfter(newStart, "new end should be after new start");
        (newEnd - newStart).TotalMinutes.Should().BeGreaterOrEqualTo(10, "duration should be at least 10 minutes");
        reason.Should().NotBeNullOrEmpty("reason should have a default value");
    }

    [Fact]
    public void RescheduleAppointmentBuilder_WithCustomValues_ShouldOverrideDefaults()
    {
        // Arrange
        var customAppointmentId = Guid.NewGuid();
        var customStart = DateTimeOffset.UtcNow.AddDays(15).Date.AddHours(10);
        var customEnd = customStart.AddMinutes(45);
        var customReason = "Custom reschedule reason";

        // Act
        var (appointmentId, newStart, newEnd, reason) = new RescheduleAppointmentTestDataBuilder()
            .WithAppointmentId(customAppointmentId)
            .WithNewTimeRange(customStart, customEnd)
            .WithReason(customReason)
            .BuildValues();

        // Assert
        appointmentId.Should().Be(customAppointmentId);
        newStart.Should().Be(customStart);
        newEnd.Should().Be(customEnd);
        reason.Should().Be(customReason);
    }
